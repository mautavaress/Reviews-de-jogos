# Regras para pedir jogo para review
Se você quer que eu jogue um jogo e faça uma análise detalhada, profunda, você está no caminho certo!

# Como pedir um jogo para review
1. Acesse o chat da [Twitch](https://twitch.tv/mautavaress/chat) ;
2. Resgate a recompensa "Review de um jogo"
3. No campo da mensagem, escreva o nome do jogo e da plataforma.

# Plataformas disponíveis
As lista de plataformas pode ser acessada por este [link](https://github.com/mautavaress/Playlist-Gamer?tab=readme-ov-file#plataformas-dispon%C3%ADveis-e-tempo-de-gameplay) (ignore a coluna "Tempo de jogo"). Também estou aceitando, em caráter temporário, pedidos de jogos que estejam no Xbox Game Pass, EA Play ou Ubisoft+ Classics, desde que:
1. Não sejam jogos como serviço;
2. Não estejam programados para deixar o catálogo dos serviços mencionados.
3. Estejam disponíveis para jogar em território nacional (alguns jogos do Game Pass só estão disponíveis em algumas regiões).
Também estou aceitando keys para jogos do Xbox.

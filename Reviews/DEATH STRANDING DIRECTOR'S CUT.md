**Plataforma jogada**: Xbox Series (nativo)

**Tempo de jogo**: `125h56min` (`123h53min41s` in-game)

**Status**: História concluída, com algumas side quests

| Critério  | Avaliação (0,5 a 10,0)|
| ------------ | ------------ |
|  Gameplay | `8,5` |
|  História |`10,0`  |
|  Visuais | `10,0` |
|  Trilha sonora e efeitos de som | `9,0` |
|  **Geral** | `9,4` |

| Pontos positivos  | Pontos negativos  |
| ------------ | ------------ |
|  Narrativa impactante |  Mecânicas de sobrevivência complexas |
| Possui muitas referências e easter eggs  | Possui imagens inapropriadas  |
|   | Side quests repetitivas  |


O jogo possui uma narrativa que é, ao mesmno tempo, boa, bizarra, e muito marcante. Fiquei muito impactado pela história. A gameplay também é boa, mas os controles são bastante complexos.

Você vai passar mais tempo contruindo coisas e fazendo entregas do que viver a história em si. Isto faz parte, pois, assim, você terá recursos para enfrentar inimigos mais perigosos e usar meios mais rápidos para se deslocar de um ponto a outro. Você provavelmente vai se viciar!

Apesar disso, recomendo a todos que, antes de jogar, abra a mente para entender o que o "Kojima e cia." querem mostrar. Caso contrário, não terão paciência alguma para jogar.